...
class MenuScene: SKScene , SKPhysicsContactDelegate{
    ...
    var aliensLeft:Int = 0 {
        didSet{
           leftLabel.text = "Wave \(wave): \(aliensLeft)"
        }
    }
    ...
    var people: [String] = ["-Lebron James", "-Barack Obama", "-Whoopi Goldberg", "-Lindsey Lohan", "-Kanye West", "-Christine Teigen", "-Carrot Top", "-Bono", "-Steve Carrell", "-Tim Tebow", "-George Washington", "-Mr. T", "-Missy Elliott", "-Gandhi", "-Yo-Yo Ma", "-Miranda Cosgrove", "-Derek Jeter", "-Justin Trudeau", "-Justin Bieber", "-Ryan Gosling", "-Kendrick Lamar", "-Michael Jackson", "-Nelson Mandela", "-Abraham Lincoln", "-Donald Trump", "-The Shins","-Vince Vaughn", "-Kyle Woodland"]
    ...
    func initialize() {
        ...
        //do { bgMusic = try SCNAudioPlayer(contentsOf: bgMusicURL as URL, fileTypeHint: nil) }
        //catch _{
       ...
       // bgMusic.numberOfLoops = -1
        ...
        pad.size = CGSize(width: (screenSize.width * 2) / 4, height: screenSize.height / 4)
        ...
        ball.size = CGSize(width: (screenSize.width * 2) / 8, height: screenSize.height / 8)
        ...
        planet = SKSpriteNode(imageNamed: "planet flabnark")
        planet.position = CGPoint(x: 0 + screenSize.width * 2, y: 0 + screenSize.height)
        planet.size = CGSize(width: planet.size.width * 4, height: planet.size.height * 4)
        planet.zPosition = -1
        ...
        bombExplosion.physicsBody?.contactTestBitMask = asteroidCategory | alienCategory | photonTorpedoCategory
        bombExplosion.physicsBody?.collisionBitMask = asteroidCategory | photonTorpedoCategory
        ...
        leftLabel = SKLabelNode(text: "Wave 1: 5")
        leftLabel.position = CGPoint(x: 100, y: self.frame.size.height - 60 - scoreLabel.frame.size.height)
        leftLabel.fontName = "AmericanTypewriter-Bold"
        leftLabel.fontSize = 36
        leftLabel.fontColor = UIColor.white
        ...
        if gameOver == false && gamePaused == false{
            if alienNumber < maxAliens{
                ...
                if alienNumber == alienStart {
                    ...
                    let random = Int(arc4random() % UInt32(3) + 1)
                    boss = random
                }
        ...
        if gameOver == false && gamePaused == false{
            for i in alienStart..<aliens.count {
                ...
                if alienDead[i] == false && random % 2 == 0{
                    let randomTorpedo = Int(arc4random() % UInt32(5) + 1)
                    let torpedoNode = SKSpriteNode(imageNamed: "torpedo\(randomTorpedo)")
                    ...
                    var actionArray = [SKAction]()
                    let dx1 = (aliens[i].position.x - spaceship.position.x) * 50
                    let dy1 = (aliens[i].position.y - spaceship.position.y) * 50
    ...
    func addAsteroid() {
        ...
        if gameOver == false && gamePaused == false{
            let asteroidNumber = Int(arc4random() % UInt32(3) + 1)
    ...
    func asteroidDidCollideSpaceship(asteroidNode:SKSpriteNode, spaceshipNode:SKSpriteNode) {
        ...
        let explosion = SKEmitterNode(fileNamed: "Explosion\(explosionRandom)")!
    ...
    func alienDidCollideAsteroid(asteroidNode:SKSpriteNode, alienNode:SKSpriteNode) {
        if alienNode.action(forKey: "start") != nil {
            ...
            if aliensHit == maxAliens {
                ...
                aliensLeft = maxAliens - alienStart
                updateTimers()
            }
        }
    ...
    func bombButtonAction(sender: UIButton!) {
        bomb.isHidden = true
        exploding = true
        let explosion = SKEmitterNode(fileNamed: "BigExplosion")!
    ...
    func updateTimers() {
        ...
        if seconds >= 0.5 {
            var tempSeconds1:Double = seconds
            tempSeconds1 -= 0.1
            seconds = 0.9 * seconds
        }
        //cap off the asteroid timer
        if seconds2 >= 0.05 {
            var tempSeconds2:Double = seconds2
            tempSeconds2 -= 0.05
    ...
    override func update(_ currentTime: TimeInterval) {
        if gameOver == false && gamePaused == false {
            ...
            let xScale:CGFloat = 0.035 //adjust to your preference
            let yScale:CGFloat = 0.035 //adjust to your preference
            ...
            //don't let stuff go off the screen
            //left
            if spaceship.position.x <= 0{
                spaceship.position.x += 5
                background.position.x -= 5/4
                planet.position.x -= 5/2
            }
            //right
            ...
            self.background.position.x -= xAdd / 4
            self.background.position.y -= yAdd / 4
            
            self.planet.position.x -= xAdd / 2
            self.planet.position.y -= yAdd / 2
                ...
                for i in alienStart..<aliens.count {
                    ...
                    while tempAlienNumb > -1 {
                        ...
                        if tempAlienNumb == alienStart {
                            ...
                            actionArray.append(SKAction.move(to: CGPoint(x: newX, y: newY), duration: TimeInterval(0.5)))
                        ...
                        if alienDead[tempAlienNumb - 1] == false {
                            ...
                            actionArray.append(SKAction.move(to: CGPoint(x: newX, y: newY), duration: TimeInterval(0.25)))
            ...
            //if the bomb is still exploding then make the circle grow
            if exploding == true {
